    ...
    for j in range(16):
        for i in range(16):
            if j == 0:
                ...
                input1.append(input_list[s][:15]+all_[i])
            else:
                input1.append(input_list[s][:j]+all_[i]+input_list[s][(j+1):])
...
for s in range(5):
    ...
    for i in range(16):
        for j in range(16):
            b.append(encrypt_4round(sets[s][i][j],key))
    ...
    for key_xor in range(16):
        for j in range(16):
            for k in range(16):
                res1=bit_xor2(ciphertexts[j][k][j],hex(key_xor).split('0x')[1])
                temp.append(S_box[res1])
            if isbalanced(temp):
                mydic[j].append(key_xor)
            temp=[]
    mydicl.append(mydic)
final={}
for i in range(16):
